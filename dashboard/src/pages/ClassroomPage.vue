...
<template>
  <section class="page-heading heading-with-status">
    <div><p class="eyebrow">Live monitoring</p><h1>Classroom activity</h1><p>See what students are doing across managed devices.</p></div>
  ...
  <section v-else class="device-grid" aria-live="polite">
    <el-card v-for="device in visibleDevices" :key="device.deviceId" shadow="hover" class="device-card" :class="{ blocked: device.blocked }">
      ...
      <div class="activity"><strong>{{ device.activity.title || "No active page reported" }}</strong><span>{{ device.activity.domain || "—" }}</span></div>
      <div class="badges">
